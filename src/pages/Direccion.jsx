...
export default function Direccion() {
  ...
  function buscarEnTodo(q) {
    ...
    categorias.forEach(cat => {
      if (cat.nombre?.toLowerCase().includes(ql)) res.push({ icono: '🗂', nombre: cat.nombre, ruta: 'Dirección', tipo: 'Categoría', accion: () => setVistaCategoria(cat) })
      proyectosDireccion.filter(p => p.categoria_id === cat.id).forEach(p => {
        ...
        subcarpetas.filter(s => s.proyecto_direccion_id === p.id).forEach(s => {
      ...
      tareas.filter(t => t.categoria_id === cat.id && t.id !== 'eliminado').forEach(t => {
        if (t.nombre?.toLowerCase().includes(ql)) {
          const p = proyectosDireccion.find(p => p.id === t.proyecto_direccion_id)
      ...
      <div className="proyectos-container">
        <div className="proyectos-header">
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            ...
            <button onClick={() => setModalEvento({ origenId: vistaSubcarpeta?.id || vistaProyecto?.id || vistaCategoria?.id, origenTipo: 'direccion', contexto: vistaSubcarpeta?.nombre || vistaProyecto?.nombre || vistaCategoria?.nombre })} style={{ background: '#7c3aed', color: 'white', border: 'none', borderRadius: '8px', padding: '8px 16px', cursor: 'pointer', fontWeight: '600', fontSize: '14px', opacity: 0.8 }}>+ Evento</button>
          </div>
        </div>

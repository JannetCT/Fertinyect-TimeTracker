...
export async function leerHoja(nombreHoja, accessToken) {
  const url = `https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/${nombreHoja.toUpperCase()}`
...
export async function escribirFila(nombreHoja, fila, accessToken) {
  const url = `https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/${nombreHoja.toUpperCase()}:append?valueInputOption=RAW`
  await fetch(url, {
    method: 'POST',
...
export async function actualizarFila(nombreHoja, id, valoresNuevos, accessToken) {
  const res = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/${nombreHoja.toUpperCase()}`, {
  ...
  if (idx === -1) return
  const filaNum = idx + 1
  await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/${nombreHoja.toUpperCase()}!A${filaNum}?valueInputOption=RAW`, {
    method: 'PUT',
    headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
